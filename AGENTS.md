...
**Conceptual references / behavioral-goal sources, clarified by the human on 2026-09-21:** HCC-A, The Coffee Cup, and Where the Water Is Loud / Dual Proofing Systems define target behavior families around cognition, causality, and communication. The former required-gearbox / holder-machinery classification is superseded by [`docs/law-why-these-documents.md`](docs/law-why-these-documents.md#conceptual-reference-documents). **laceArc is the experimental lab: propose, attack, measure, and retain only the systems actually required to make Lace function as described by those behavioral goals.** [`docs/clock/philosophy-map.md`](docs/clock/philosophy-map.md) and the clock diagrams are interpretations/candidates, not preinstalled machinery. **What precisely enables those behaviors remains undetermined.** H/C/W labels do not establish required holder components, runtime dependencies, fixed enabling mechanisms, or legal-append conditions. Ledger mutation and OBS remain excluded as Graphic D operations or row types. Conceptual analogy does not supply pointer emission.
