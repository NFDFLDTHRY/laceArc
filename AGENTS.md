...
**Current behavioral-machine court — Behavioral Read Devices Passes 1–6:** [`docs/math-execution-model.md`](docs/math-execution-model.md) S9–S12 now carries the current non-Core machine mathematics. S9 is the minimal read-device algebra `(Q,G,Ω,Δ,ω)` and device-only work has no Lace write authority; S10 separates evidence classes and non-identifiability; S11 separates technical realization/evidence from semantics; Pass 4 repairs Root/Star/Reach/path/projection fidelity; S12 admits only an **optional** local operator library (`Par`, candidate generation, check, bounded advance, stop/control, resolve) and explicitly rejects a fixed common chassis. Executed mathematical blueprints exist for [HCC-A](docs/plans/math-execution-behavioral-read-devices-pass-5-hcc-device-blueprint.md), [Coffee Cup](docs/plans/math-execution-behavioral-read-devices-pass-5-coffee-cup-device-blueprint.md), and [Water](docs/plans/math-execution-behavioral-read-devices-pass-5-water-device-blueprint.md). They are **not implementations, Core components, or acceptance**. Exact H×C×W runtime coupling remains OPEN.

**Water current human ruling:** the raw source's blocking-UNK / lexical-INVALID reading is historical provenance, not the current target. Current UNK is a **nonblocking predicted evidence shape** waiting for future evidence; current INVALID is a **claim later evidence has established as wrong**. There is no current invalid-phrase table. See the Water clipboard and live law.
