...
Clock: objects do not have relationships. The premise is structure. Clusters form as the train ticks. A variable is a label on observed index geometry of Graphic D — a view, not a second array, not pointer-emission. When the observed cluster changes, the label may be created, split, or merged. Those acts name. They do not rewrite WORD @ i. The cat fragment is the specimen; do not invent `>`.
