...
**Status:** Pass 1 EXECUTED. Emission `[GAP]`.  
...
| False friend | FM-T1 — WASM module ⇒ Lace crate / Core door |
...
| False friend | FM-T2 — valtype ⇒ WORD/POINTER kinds |
...
| False friend | FM-T3 — stack-type matching ⇒ POINTER when/arity |
...
| False friend | FM-T4 — linear memory / store / table / stack ⇒ Lace 1D strand / Graphic D |
...
| False friend | FM-T5 — `br` / `return` / calls ⇒ POINTER emission / Φ |
...
| Lace stamp | **KEEP-read-as-target** |
| False friend | FM-T6 — decode success ⇒ emission accepted / src/ opened |
...
stamp under fence: KEEP-read-as-target
...
| False friend | FM-T7 — `.wat` sugar ⇒ second Core syntax / emission law |
...
| False friend | FM-T8 — import host fn fills emission `[GAP]` |
...
| Lace stamp | **KEEP-read-as-target**; **FORBID-as-emission-rule**; **SILENT-for-Lace-append** |
| False friend | FM-T9 — trap/divergence ⇒ Lace append rule / POINTER when |
...
| Panel | Clipboard PAGE | Primary tooth |
|---|---|---|
| P-T1 | B | FORBID-as-Lace-store / FORBID-as-Core |
| P-T2 | C | FORBID-as-Core-write |
| P-T3 | D | FORBID-as-emission-rule |
| P-T4 | E | **FORBID-as-Lace-store** (linear mem ≠ strand) |
| P-T5 | F | **FORBID-as-emission-rule** (`br`/`return` ↛ Φ) |
| P-T6 | G | KEEP-read-as-target decode |
| P-T7 | G | text non-authority twin |
| P-T8 | H | **PROPOSAL-only** bridge; GAP remains |
| P-T9 | X | SILENT-for-Lace-append; traps ≠ append |
