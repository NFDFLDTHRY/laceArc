...
**Status:** **`PLAN`** · not executed. **Station:** clipboards (HELD `Claude-Projection` · BASE `014a8a3` = `origin/main` after `pull --ff-only`). **Emission:** `[GAP]`. No `src/`.
