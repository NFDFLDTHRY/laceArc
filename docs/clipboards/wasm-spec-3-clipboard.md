...
## Repository update pass 2 — target sightings

**2026-09-20; base `10fe4947`.** Codex-References rendered and visually inspected the supplied `09-WebAssembly.pdf`, Release 3.0 (2026-09-11), SHA-256 `687e73641a7eec8692273fdf048e1578bf2f379f507c36ec284a39d9e7f70fce`. All PDF page numbers here are one-based. The inspected pages were **42, 188–190, 205–206, 209, 253–255, 274, 282–284** (14 pages); this is a targeted sighting, not a full-specification reread or execution of a prior campaign.

| ID / request | Visually sighted source | Established source fact | Limit of the carry |
|---|---|---|---|
| W2-S1 — G9 memory address type | §5.3.9 Limits, printed 184 / PDF 188; §5.3.12 Memory Types, printed 185 / PDF 189; §5.5.8 Memory Section, printed 202 / PDF 206 | The limits grammar uses `0x00` / `0x01` for i32 and `0x04` / `0x05` for i64, respectively without / with a maximum. Memory types encode their limits; section id 5 contains locally defined memories | These are grammar-positioned flags, not arbitrary bytes to search. Imported memories use the external-type/import grammar (§5.3.14 / §5.5.5, PDF 189/206). No actual module was decoded and no engine capacity was measured |
| W2-S2 — memory validation | §3.2.12 Limits and §3.2.15 Memory Types, printed 38 / PDF 42 | Bounds must be ordered and within the stated range; a memory type's limits must validate within `2^(b−16)`, where `b` is the address type's bit width | This is a specification rule, not an observation that a selected checker enforces it or that a device can allocate that amount |
| W2-S3 — b3 name carrier | §5.5.2–5.5.3 Sections / Custom Section, printed 201–202 / PDF 205–206; §7.7.1 Name Section / Name Maps / Function Names, printed 278–280 / PDF 282–284 | Custom sections have id 0 and do not affect Wasm semantics. The custom section named `name` carries optional naming subsections; function names map function indices to names. Names need not be unique and the metadata may be ignored | The carrier format is now sighted. Its presence, absence or a negative crate-root-name scan does not establish complete Rust-code provenance. `strip=none`, v0 naming and the coverage of surviving names still need an exact artifact/toolchain argument; this source alone cannot prove absence of a crate's code |
| W2-S4 — G8′ exception constructs | §5.3.10 Tag Types, printed 184 / PDF 188; §5.3.14 External Types, printed 185 / PDF 189; §5.4.2 Control Instructions, printed 185–186 / PDF 189–190; §5.5.5 Imports, printed 202 / PDF 206; §5.5.16 Tags, printed 205 / PDF 209 | Local tags use section id 13; external types also include tags, so a tag can be imported. Control encodings include `throw` (`0x08`), `throw_ref` (`0x0A`) and `try_table` (`0x1F`) | Absence of a local tag section does not exclude imported tags. A literal “try/throw” text or byte scan is not the grammar check. These sightings identify required distinctions; they do not certify an exhaustive EH-exclusion policy or a validator's disabled-feature behavior |
| W2-S5 — proposed feature subset | §7.2 Profiles / Conventions / Defined Profiles and §7.3 Implementation Limitations, printed 249–251 / PDF 253–255 | Profiles describe coherent syntactic/semantic subsets; the defined profiles here are FUL and DET. Section 7.3 separately states that a conforming implementation cannot omit individual features | G8′ is a proposed restriction on acceptable project artifacts. It is not established as one of these profiles or as a conforming engine's right to omit features. The intended subset, its relation to an actual validator mode and the tool's switches remain separate evidence obligations |
| W2-S6 — validator algorithm | §7.6 opening, printed 270 / PDF 274; the `try_table` continuation was visible at printed 278 / PDF 282 | The appendix presents a typed-pseudocode skeleton for validating instruction sequences; the specification's constraints are declarative | Only these bounded pages were visually checked, not the complete algorithm. No checker implementation, feature configuration, soundness proof for G8′ or validation run was inspected or executed |

**Current disposition of the Page G request:** binary memory/tag/operator grammar and the optional name carrier are `SOURCE / VISUALLY_OBSERVED` within the cited sections. Name-based crate absence remains **UNESTABLISHED**; feature-policy enforcement and artifact results remain **NOT_RUN**. These distinctions qualify any earlier reading that a source carrier alone made B2/b3 or G8′ a completed check. The [rust-target addendum](rust-target-clipboard.md#repository-update-pass-2--target-evidence-carry) records the affected rows without rewriting its historical receipt.

Cargo **A4 remains next**. The rustc source text, Cargo A4, JS-API A3, rustup A10 and tool-conventions A11 are absent from the supplied twelve-file corpus. This pass does not recover their contents, choose a tool, take R1/R11/R12, authorize a fixture, or run a build. No selected artifact, browser, device or physical evidence was produced. No supplied PDF was changed or added to Git. Wasm memory and its types remain target semantics, not Lace's authoritative store or an emission rule.
