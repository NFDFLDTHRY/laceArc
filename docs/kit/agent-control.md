...
The block above is the author's law, preserved verbatim: a **transport copy** of the fenced block in [`docs/law-why-these-documents.md`](../law-why-these-documents.md), regenerated byte for byte at revision `e21cf39`. Where this copy and the live law differ, the live law wins. Everything below is a steward operating contract for obeying it. Nothing below supplies a missing Core rule, accepts pointer emission, or proves that Lace works. This document governs the agents helping build Lace; it is not an agent architecture inside Lace.
