...
**Current steward qualification — ROOT / TOUCH / FOLD Pass 5:** the verbatim transport block above is historical/human law text and is not rewritten here. Current repository law now records WORD landing, unseen→root, seen→RootTouch, binary `Join(a,b)`, earlier same-line WORD/POINTER operands, and finite-sample grounding in a **reconciled but unaccepted** pointer draft. **Arbitrary non-root-touch relation/sample selection remains `[GAP]`.** Reconciliation is not human acceptance and does not open `src/`, Cargo, or an implementation branch.
...
The current constructor questions are no longer one broad bundle. Binary Join, RootTouch, backward same-line WORD/POINTER operands, finite-sample grounding, and adjacency-alone=false are reconciled draft-law facts. **Still open:** arbitrary non-root-touch relation/sample selection, G1 word boundaries/identity, read/traversal behavior, concurrent arrival ordering, route-family/boundary questions, and task-specific implementation premises. Cite the exact source of each question. Do not bundle them into one invented selector.
