...
The block above is the author's law, preserved verbatim: a **transport copy** of the fenced block in [`docs/law-why-these-documents.md`](../law-why-these-documents.md), regenerated byte for byte for the human's 2026-09-20 conceptual-reference correction (base `9907980`; see the [reconciliation record](../plans/conceptual-reference-reconciliation.md)). Where this copy and the live law differ, the live law wins. Everything below is a steward operating contract for obeying it. Nothing below supplies a missing Core rule, accepts pointer emission, or proves that Lace works. This document governs the agents helping build Lace; it is not an agent architecture inside Lace.

**Manifest Reconciliation Pass 2 foundation:** use the Pass-32 global mechanism catalog for mechanism existence/connections and the systems-manifest contract registry for mechanism behavior/boundaries. Current source routing is A6=context/document, B4=explicit definition route, B10=no-collapse, B11=projection; persistence/re-entry is distinct from retention/materialization; Graphic-C 2D projection is distinct from H7–H12 sampling. These are operating qualifications, not new Core rules.

**Current steward qualification — Pass 39 / repo-lag Pass 3:** the verbatim transport block above is historical/human law text and remains byte-identical to the live-law fenced block at the Pass-3 pre-edit tip `0bd0b326`; it is not rewritten here. The current repository court is the unchanged S0–S8 Lace substrate plus non-Core **S9 read-device**, **S10 observability**, **S11 realization/evidence**, and **S12 optional device-local operator** mathematics. S12 rejects a fixed common chassis. Current HCC-A / Coffee Cup / Water mathematical blueprints realize source obligations at the device layer but are **not runtime implementations**. Preserve the distinction **source desired-behavior mechanism != concrete implementation mechanism**.

**Current open/gate state:** the pointer draft records WORD landing, unseen→root, seen→RootTouch, binary `Join(a,b)`, finite grounding, and the human-ruled derived sampling/search architecture, but remains **unaccepted**. **RM-A retain-when is OPEN / MISSING-A; G1 OPEN; R3 OPEN; GAP-HOST OPEN; exact H×C×W runtime coupling OPEN; compilation target TARGET UNDECIDED; no `src/`.** Bounded RM-B/RM-C results stay scoped Lab evidence. Current Water target semantics are **UNK = nonblocking predicted evidence shape** and **INVALID = prior claim later refuted by pointable evidence**; older blocking-UNK / lexical-INVALID readings survive only as source/history.

**Lab tooling fence:** `docs/plans/tools/lace-candidate-run.py`, `lace-retention-harness.py`, the D1 golden fixture, candidate admission envelopes, falsifier boards, and their findings are **Lab instruments/evidence**. They may kill, qualify, or preserve candidates. They do not write Lace, do not become dependencies, and do not gain Core authority from a passing score. **Lab “retain” means retain evidence/candidate status; Lace “retain” means durable POINTER topology.**
...
The gate now reads: `docs/pointer-emission.md` exists as a reconciled draft, but Core implementation still requires identifiable **human acceptance of that exact content** plus every other premise needed by the task. The draft settles the current constructor mechanics and records the human-ruled sampling/search architecture. **It does not settle RM-A retain-when.** Bounded RM-B/RM-C closures do not waive acceptance or candidate-specific proof. Acceptance of one document never accepts every future design choice.
...
Writing prose, auditing sources, and preparing reviewable contracts are allowed. Do not populate the remaining selection gap or other open questions with defaults. This control prompt is not `docs/pointer-emission.md`; changing or accepting this prompt does not accept the pointer contract.
...
The current questions are no longer one broad bundle. Binary Join, RootTouch, backward same-line WORD/POINTER operands, finite-sample grounding, the derived 2D sampling surface, and read-only star/search frontiers are reconciled current facts. **Still open generally:** RM-A retain-when, candidate classes outside the bounded RM-B/RM-C closures, G1 word boundaries/identity, general traversal/geometry questions outside mapped star search, concurrent arrival ordering, route-family/boundary questions, and task-specific implementation premises. Cite the exact source and scope of each question. Do not bundle them into an invented selector or retention heuristic.

The September 6 `ATOM`/unary `POINT(target)` prompt is historical material. Current law now reconciles the governing POINTER constructor as **binary Join(a,b)** using D2/D1 plus explicit human rulings. The old unary prompt remains rejected history; it does not reopen arity or become implementation authority. Do not rename `WORD` to `ATOM` as if that were cosmetic.
...
WORD and POINTER are the entries drawn in Graphic D. A STAR is accumulated formation, not a separately stored object. Current draft law constrains POINTER to binary Join over two ordered earlier same-line WORD/POINTER operands; seen occurrences RootTouch their root. Later human rulings make sampling the discovery mechanism. **Do not invent RM-A, universalize the bounded RM-B/RM-C results, add a semantic operand chooser, or invent implementation ownership.**
...
Necessary obligations already implied by current law include preserved prior entries, retained occurrence order, binary Join over existing earlier positions, derived multi-scale sampling, and no authoritative side store. They still do not decide **RM-A retain-when**, acceptance, or task-specific implementation proof. Bounded RM-B/RM-C evidence applies only to its declared candidate classes.
