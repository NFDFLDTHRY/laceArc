...
**Current steward qualification — SAMPLING / DENSITY / EMERGENCE Pass 6:** the verbatim transport block above is historical/human law text and is not rewritten here. Current repository law records WORD landing, unseen→root, seen→RootTouch, binary `Join(a,b)`, one authoritative 1D Lace, and the later human-ruled derived 2D sampling/search architecture in a **reconciled but unaccepted** pointer draft. **Exact sampling-derived retention/materialization remains `[GAP]`.** Search is derived/read-only; reconciliation is not human acceptance and does not open `src/`, Cargo, or an implementation branch.
...
The current questions are no longer one broad bundle. Binary Join, RootTouch, backward same-line WORD/POINTER operands, finite-sample grounding, the derived 2D sampling surface, and read-only star/search frontiers are reconciled current facts. **Still open:** exact sampling-derived retention/materialization, G1 word boundaries/identity, general traversal/geometry questions outside the mapped star search, concurrent arrival ordering, route-family/boundary questions, and task-specific implementation premises. Cite the exact source of each question. Do not bundle them into an invented selector or retention heuristic.
