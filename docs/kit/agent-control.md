...
**Current steward qualification — Alignment Restart Pass 1:** the verbatim transport block above is historical/human law text and is not rewritten here. Current repository law records WORD landing, unseen→root, seen→RootTouch, binary `Join(a,b)`, one authoritative 1D Lace, and the human-ruled derived 2D sampling/search architecture in a **reconciled but unaccepted** pointer draft. The canonical `SM-GAP-RM` interface remains open, but latest bounded Lab evidence narrows its faces: **RM-A retain-when remains open / MISSING-A; RM-B is partially closed for selected finite contiguous samples; RM-C is class-specifically closed for RootTouch and contiguous sample grounding.** Search is derived/read-only; these Lab results are not universal Core law and do not open `src/`, Cargo, or an implementation branch.
