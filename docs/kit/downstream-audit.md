...
| F10 | Pointer-emission.md is absent; the manifest also leaves identity, reads, concurrency, and geometry encoding open. | "Pointer file accepted" is mistaken for a complete runtime contract. | Acceptance enables only its settled scope; dependent gaps still block. C03-C04/C08. |
