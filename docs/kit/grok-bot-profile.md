...
Repo law (same ASCII, with preamble): [`docs/law-why-these-documents.md`](../law-why-these-documents.md). The Explanation block below is a **transport copy** of its fenced block, regenerated byte for byte for the human's 2026-09-20 conceptual-reference correction (base `9907980`; see the [reconciliation record](../plans/conceptual-reference-reconciliation.md)). Where they differ, the live law wins.

**Pass-3 transport recheck, 2026-09-22:** the Explanation fence below is still byte-identical to the live-law fenced block at pre-edit tip `0bd0b326`. Its transport provenance remains dated; the surrounding provider wrapper is current. After AGENTS, read [`context-pass.md`](../prompts/context-pass.md) and the [live diagram prompt](../prompts/lace-context-diagram-prompt.md). Current machine/gate state: S9–S12 are non-Core overlays; RM-A/G1/R3/GAP-HOST and exact H×C×W coupling remain open; compilation target is **TARGET UNDECIDED**; pointer acceptance is absent.
...
After AGENTS.md and before the first task, read `docs/prompts/context-pass.md` and `docs/prompts/lace-context-diagram-prompt.md` — the current Pass-39-era story and visual court every agent should share. Sign every commit (`docs/kit/roles.md`). Hold a station only as long as you said you would.

## First task

```
Outcome: Ground yourself on the CURRENT laceArc tree and return a one-page steward brief. Do not choose the next repository task.
...
Constraints:
- Pull current main and report the exact HEAD you read.
- Do not create src/.
- Do not invent RM-A, tokenization, R3, GAP-HOST, target selection, H×C×W scheduling, acceptance, or any replacement pointer rule.
- Do not add crates.io or any language other than future Rust no_std when implementation is ever authorized.
- Do not push unless I say push.
- If GitHub login or clone needs takeover, stop and ask.
- Propose no new file and no next campaign unless the human assigned one.

Deliverable:
1. Confirm repo HEAD commit.
2. Restate the one-strand rule in both voices.
3. Separate Core S0–S8 from non-Core S9 / S10 / S11 / S12.
4. State RM-A, G1, R3, GAP-HOST, exact H×C×W coupling, TARGET UNDECIDED, pointer acceptance, and src/ status.
5. State the current HCC-A / Coffee Cup / Water role: source desired-behavior mechanisms with mathematical device blueprints, not fixed runtime components.
6. State the current Water UNK / INVALID ruling.
7. State the assigned task boundary. If none was assigned beyond grounding, say NONE.

Review point: stop after the brief. Do not schedule work and do not manufacture a “next file.”
```
