...
*Superseded on these points:* R5 **Q2**, **Q3**, **Q5**; R4 *II → Φ: does not fill* (reads: Arrival does not fill RM-A; the RootTouch that follows a seen arrival is Core's mandatory append, S3.3/S4.4, not Arrival's act); R7's `Φ [GAP]` reads as `RM-A [GAP]`; the Status line's *Pass 2 Contract II* (S5 is current). The Emission line's **G1 tokenization: still open** stands.
*Stands as written:* R0's law — feed ≠ store, feed ≠ emit, arrival ≠ projection; R1; R2; R3 (Pieces 9–10 as use of I); R5 **Q1**, **Q4**; R6; R7. *Every successful arrival* now has its refusal path pointed from the model (S2, S5: REFUSE under R4 in the reduction execution model; the lab-source Pass-6 capacity / refusal contract); this file's silence on refusal was never a claim that arrivals cannot be refused.
