# Xiao & Zhu *Foundations of Large Language Models* — ASCII diagram agent prompt

**Classification, 2026-09-22: HISTORICAL / COMPLETED CREATION TICKET.** The filled [Xiao–Zhu product](../clipboards/xiao-zhu-foundations-clipboard.md) already exists and has completed source-mapping passes. Do **not** replay this prompt to recreate or overwrite that product. Preserve the body below as the original source-mapping task. For current research-machine use, read [the clipboard shelf](../clipboards/README.md#current-research-machine-role--behavioral-read-devices-pass-56), S9/S12, and the existing product; source machinery may support optional device-local operators but remains non-Core and source-specific.


Paste everything inside the outer box to an agent. Shadow only. Not Core. Emission `[GAP]`. Never commit the PDF.

Source: Tong Xiao & Jingbo Zhu — *Foundations of Large Language Models* · arXiv:2501.09223v2 · CC BY-NC 4.0  
Hands first: shoe → graphics A–D → systems-manifest → staking / law-why → reason-model-map §3 → then the PDF (attachment / refs/local only).

```
╔══════════════════════════════════════════════════════════════════════════╗
║  LACEARC  ·  ASCII PROMPT  ·  XIAO & ZHU CLIPBOARD                       ║
║  Foundations of Large Language Models · arXiv:2501.09223v2               ║
║  Shadow stake only · Hands win · POINTER emission = [GAP]                ║
╚══════════════════════════════════════════════════════════════════════════╝

  YOU ARE NOT implementing Core.
  YOU ARE NOT inventing emission.
  YOU ARE filling ASCII clipboard sheets that map THIS book onto the shoe.


                    ┌────────────── HANDS ──────────────┐
                    │  shoe lace (two ends)               │
                    │  docs/graphics A B C D              │
                    │  docs/systems-manifest.md           │
                    │  staking + law-why                  │
                    │  reason-model-map.md §3             │
                    └──────────────────┬─────────────────┘
                                       │ read first
                                       v
                    ┌────────────── SHADOW ─────────────┐
                    │  Xiao & Zhu Foundations of LLMs     │
                    │  (attachment / refs/local only)     │
                    │  NEVER git-add the PDF              │
                    │  Cite; do not fold body into Core   │
                    └──────────────────┬─────────────────┘
                                       │
                                       v
                         FILL CLIPBOARD PAGES BELOW


┌──────────────────────────────────────────────────────────────────────────┐
│ CLIPBOARD                                                                │
│ laceArc ← Xiao & Zhu Foundations of LLMs                                 │
│ law: arrival order kin · intern/MASK/embed/next-token ≠ Core · [GAP]     │
├──────────────────────────────────────────────────────────────────────────┤
│ [clip]                                                                   │
│                                                                          │
│   ┌────────────────────────────────────────────────────────────────┐     │
│   │ PAGE A — TOKEN SEQUENCE / ARRIVAL ORDER                        │     │
│   │ locus: Ch.___  §____  p.___                                    │     │
│   │ quote/eye: _______________________________________________     │     │
│   │                                                                │     │
│   │   ordered tokens ──looks like──► Piece 3 arrival               │     │
│   │   (kin only while tokens are not interned ids)                 │     │
│   │                                                                │     │
│   │ ON SHOE: Piece 3 Raw Input / Arrival Order                     │     │
│   │ KEEP: arrival order                                            │     │
│   │ FORBID: treat vocab atoms as Lace WORD identity                │     │
│   │ EMISSION? NO [GAP]                                             │     │
│   │ vs reason-model-map §3: confirms|refines|conflicts|silent      │     │
│   └────────────────────────────────────────────────────────────────┘     │
│                                                                          │
│   ┌────────────────────────────────────────────────────────────────┐     │
│   │ PAGE B — INTERNED VOCABULARY TOKEN                             │     │
│   │ locus: §____  p.___                                            │     │
│   │                                                                │     │
│   │   one id "THE" @ many positions ──X──► two wraps of "the"      │     │
│   │   (Graphic B forbids intern collapse)                          │     │
│   │                                                                │     │
│   │ KEEP: (none as Core primitive)                                 │     │
│   │ FORBID: intern token as WORD / Star node                       │     │
│   │ ON SHOE: Piece 4 / Piece 7 · Graphic B                         │     │
│   │ EMISSION? NO [GAP]                                             │     │
│   └────────────────────────────────────────────────────────────────┘     │
│                                                                          │
│   ┌────────────────────────────────────────────────────────────────┐     │
│   │ PAGE C — [MASK] / REPLACE / DELETE (BERT-style)                │     │
│   │ locus: §1.2.2 / BERT  p.___                                    │     │
│   │                                                                │     │
│   │   mask · random-replace · leave ──X──► Rule Zero / Piece 1     │     │
│   │   "It is raining" → "It is [MASK]" = cut-and-retie             │     │
│   │                                                                │     │
│   │ KEEP: (none — anti-model for tokenizer notes)                  │     │
│   │ FORBID: [MASK] / replace / delete as Lace write                │     │
│   │ ON SHOE: Piece 1 continuous strand · Piece 3 keep the pass     │     │
│   │ EMISSION? NO [GAP]                                             │     │
│   └────────────────────────────────────────────────────────────────┘     │
│                                                                          │
│   ┌────────────────────────────────────────────────────────────────┐     │
│   │ PAGE D — EMBEDDING STORE                                       │     │
│   │ locus: Ch.2  §____  p.___                                      │     │
│   │                                                                │     │
│   │   token+pos+segment vectors ──X──► second store beside cord    │     │
│   │                                                                │     │
│   │ KEEP: (none as Core schema)                                    │     │
│   │ FORBID: embed store / rotary table as Lace authority           │     │
│   │ ON SHOE: Graphic D — no separate database                      │     │
│   │ EMISSION? NO [GAP]                                             │     │
│   └────────────────────────────────────────────────────────────────┘     │
│                                                                          │
│   ┌────────────────────────────────────────────────────────────────┐     │
│   │ PAGE E — NEXT-TOKEN / CAUSAL LM AS CORE                        │     │
│   │ locus: §1.2.1  p.___                                           │     │
│   │                                                                │     │
│   │   Pr(next | prefix) ──X──► Piece 2 Continue (append section)   │     │
│   │   distribution is a projection that talks about a lace         │     │
│   │                                                                │     │
│   │ KEEP: (none as Core operator)                                  │     │
│   │ FORBID: next-token loss / decoder objective as Lace Core       │     │
│   │ ON SHOE: Piece 2 output = new section, not a distribution      │     │
│   │ EMISSION? NO [GAP]                                             │     │
│   └────────────────────────────────────────────────────────────────┘     │
│                                                                          │
│   ┌────────────────────────────────────────────────────────────────┐     │
│   │ PAGE F — ANYTHING THAT LOOKS LIKE “WHEN TO POINT”              │     │
│   │ locus: §____  p.___   or NONE FOUND                            │     │
│   │                                                                │     │
│   │   looks like emission ──silenced──► [GAP]                      │     │
│   │   do not invent when / arity / adjacency                       │     │
│   │   (tokenization remains unstated in graphics)                  │     │
│   │                                                                │     │
│   │ DELTA: silent | false friend | conflict                        │     │
│   └────────────────────────────────────────────────────────────────┘     │
│                                                                          │
│   ┌────────────────────────────────────────────────────────────────┐     │
│   │ PAGE X — CROSS-CUT                                             │     │
│   │                                                                │     │
│   │  temptation              │ Hands deadbolt                      │     │
│   │  ───────────────────────┼──────────────────────────────────── │     │
│   │  ordered token list     │ Piece 3 arrival (raw, keep pass)     │     │
│   │  interned vocab id      │ two wraps ≠ one node named THE       │     │
│   │  [MASK] / replace       │ Rule Zero — no cut-and-retie         │     │
│   │  embedding side store   │ Graphic D one line                   │     │
│   │  next-token Core        │ Continue = append, not Pr(·)         │     │
│   │  emit POINTER from book │ [GAP] — book does not settle it      │     │
│   │                                                                │     │
│   │  STILL [GAP]: POINTER when / arity / adjacency                 │     │
│   │  STILL [GAP]: how spelling becomes WORD without intern/mask    │     │
│   └────────────────────────────────────────────────────────────────┘     │
│                                                                          │
│ [clip]                                                                   │
└──────────────────────────────────────────────────────────────────────────┘


  FLOW (ASCII)

       fetch/ff main ──► shoe ──► graphics ──► manifest ──► law
              │
              v
         reason-model-map §3
              │
              v
         open Xiao & Zhu PDF (no git add)
              │
              v
         fill PAGE A → B → C → D → E → F → X
              │
              v
         write docs/clipboards/xiao-zhu-foundations-clipboard.md
              │
              v
         STOP  (commit only if human asks)


  FORBIDDEN
    invent emission · add src/ · PDF in git
    intern token as WORD / Star table
    [MASK] / replace / delete as Core write
    embed store or next-token objective as Core
    paraphrase away KEEP arrival order


  STEWARD TEST
    shoe lace in hands, or no.
    crate did it for you ⇒ you may not.


╔══════════════════════════════════════════════════════════════════════════╗
║  END ASCII PROMPT — fill the clipboard pages; leave [GAP] sealed         ║
╚══════════════════════════════════════════════════════════════════════════╝
```
