# Rust target clipboard — agent prompt (tenth shelf, staked by the human)

Agent ticket. Not Hands. Not law. Not Core. POINTER emission stays `[GAP]`. No `src/` from a prompt.

**Staked by the human** 2026-09-19 22:54 PT by attaching *The rustc book* as a text conversion (his words: *"I was forced to convert to text to get your interface to accept the technical reference"*). This is the **A1** reference of [iteration 6 pass 2 §3](../plans/lace-context-iter6-pass-2-findings.md). A shelf exists because the human staked a document; the sealed refuse against agents *inventing* a tenth shelf is honored.

## How a reference is encoded here (the rule the nine shelves already follow)

```
  the document          →  refs/local/<name>   (gitignored; never git-add; the human keeps it)
  its identity          →  title · version if stated · size · line count · SHA-256 of the exact file attached
  the section that matters  →  line locators into that exact file
  what it says          →  short verbatim "eye entries", never a dump; paraphrase marked as such
  what it means here    →  KEEP-read-as-target · FORBID-as-Core · SILENT-for-Lace stamps
  what it changes       →  which stamped claims in the tree it upgrades from web-OBSERVED to SOURCE
