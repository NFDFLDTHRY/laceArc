...
**Current reconciliation:** 2026-09-20, [repository update pass 4](../plans/repo-update-pass-4-evidence.md#10-execution-reanchor-and-choices), executed locally against published pass3 at `335429c`. Pass3's thirty files were published in eight station commits; its [earlier local receipt](../plans/repo-update-pass-3-evidence.md), [pass 2](../plans/repo-update-pass-2-evidence.md) and [pass 1](../plans/repo-update-pass-1-evidence.md) remain dated records. Current evidence distinguishes lifecycle/cache/control repairs, source corrections and synthetic checks from pending browser, device and designated-renderer work. No model or emission acceptance is asserted; source publication does not promote the pinned ixp2 release.
...
| [the compute model + inventory](../plans/lace-context-iter7-pass-2-compute-and-inventory.md) | proposed one-writer/frontier arrangement (R9); WebGPU/WGSL as projection; **WebNN has no Core role** (R10); import absence is a structural check, not F-S3's behavior proof; dated 32-component proposal with scoped gates and unassigned σ implementation ownership | `[PROPOSAL]`; R5/R6/R9 and the submit/word boundary remain unresolved |
...
8. **A candidate rule** runs through all seven steps of [E7](../plans/lace-context-iter6-pass-4-environment.md#e7--the-procedure-what-an-agent-runs). Evidence for all thirteen predicates must cover their existing E2/E3 domains, including F-G/F-B outside D1. Show every produced row beside D1’s eleven, treating missing or extra rows as mismatches; success supplies F-N, while missing evidence does not prove refutation. Keep final-gap and per-STEP F-H5 evidence distinct under the stated S4.3 premise. E7 permits one declared agent; its stated independence check is a second agent rerunning D1. IN-REGION remains `[PROPOSAL]`, with acceptance ABSENT.
...
*One strand, one rule, one store; the graphics are the source and the law says what may exist. Lace Core implementation awaits the human's acceptance of pointer emission; Layer III viewers and steward tools already exist. The missing rule has a type and a fenced region, with no recorded candidate established in-region. The proposed running model separates authoritative strand history from pending-input and carrier state, and its proofs require explicit lifecycle and observation scopes. The target shape and production goals are proposals with open rulings; A1 is mapped and A4 has a bounded source mapping; no build or deployment success follows from either. The twenty-one-document door is a dated base, extended by current source and consumer evidence. Work within the authorized pass, one station per commit, signed, stamped and mapped; preserve source conflicts and stop at the named boundary. The shoe is the court; emission remains `[GAP]`.*
