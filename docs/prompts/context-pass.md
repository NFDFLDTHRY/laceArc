...
Layer III steward prompt. Not Hands. Not law. Not Core. **Current constructor mechanics are reconciled in an unaccepted draft; arbitrary non-root-touch relation/sample selection remains `[GAP]`.** No `src/` from a prompt.
...
- **The old broad sigma hole has been narrowed.** The dated [environment](../plans/lace-context-iter6-pass-4-environment.md) typed `σ : 𝓛 × V → Step*`, fenced it, and classified 17 historical candidates. Keep that ledger intact. Later human rulings resolved LAND/root/RootTouch/binary Join/finite-sample grounding. The remaining Core-adjacent hole is the **upstream arbitrary non-root-touch relation/sample selector**. No current source defines `Select(L,v)=...`; do not revive an old candidate as that selector.
...
| [the compute model + inventory](../plans/lace-context-iter7-pass-2-compute-and-inventory.md) | proposed one-writer/frontier arrangement (R9); WebGPU/WGSL as projection; **WebNN has no Core role** (R10); dated 32-component proposal. Its old broad σ ownership question is now read as the still-unassigned arbitrary relation/sample selector | `[PROPOSAL]`; R5/R6/R9 and the submit/word boundary remain unresolved |
...
| [the companion](../systems-manifest-ascii.md), Passes 26–29 and current scope notes | Pass 29 carries the current ROOT / TOUCH / JOIN law into the global systems reading while retaining older opens/history; M1's shared-strand-history claim keeps its observation/lifecycle scope | readings, not new canonical sources or a universal app-state proof |
...
*One strand, one rule, one store; the graphics are the source and the law says what may exist. The current draft records WORD landing, root/RootTouch, binary Join and finite-sample grounding; dot-to-dot N-D remains projection of the same line. **The draft is not human-accepted and arbitrary non-root-touch relation/sample selection remains `[GAP]`.** Layer III viewers and steward tools already exist, but they do not choose Core relations. The dated sigma environment and candidate ledger remain evidence, not today's constructor status. The proposed running model separates authoritative strand history from pending-input and carrier state, and its proofs require explicit lifecycle and observation scopes. Work within the authorized pass, one station per commit, signed, stamped and mapped; preserve source conflicts and stop at the named boundary. The shoe is the court; known constructor does not mean accepted implementation or known selector.*
