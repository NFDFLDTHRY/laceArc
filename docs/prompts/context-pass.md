...
8. **If proposing an arbitrary selector**, do not treat the old E7 scheduler search as current law. E7 remains a dated adversarial procedure and its 17-candidate evidence stays useful for rejecting old scheduler ideas. Any new selector proposal must be explicit about its inputs, avoid forbidden semantic/learned/side state, preserve current Join law, and remain `[PROPOSAL]` until the relevant authority accepts it. Acceptance of the pointer contract is still ABSENT.
