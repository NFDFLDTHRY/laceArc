...
| **4** ✅ | maps + law | **DONE** — see §4.8. `docs/namespace-register.md`, and links to it. **Superseded in §4.7: no renaming.** The keys are baked into `contracts-*.js` and the IR fixtures, `A1` alone spans 50 files, and the collision has caused no recorded error. Register + citation rule instead. | done |
| **5** ◑ | hologram + coord + maps + clipboards | **DONE except 5-pre** — see §4.10. Re-scoped in §4.9 — the shelf grew an app, and R4 was too strong. Three gear manifests retitled `Gear manifest — HCC-A (Shadow)` etc. (title line only; filenames stay). `clock/passes/` + index for the executed pass plans (24 and counting), `git mv` only. **Absorbs sub-phase 1i:** the clock door is written here, once, against the folded shape. Forward rule in `clock/README.md`: no new one-file-per-pass. | go phase 5 — **after the Cup/Water/HCC campaign stops** (passes landed within the last hour) |
| **6** ✅ | kit | **DONE** — see §4.12. Re-scoped in §4.11: nothing moves. The move was wrong: downstream-audit is live, not historical, and R10 was already closed by 1g. One dated row remains. Original text: | `READ_ME_FIRST.md` and `downstream-audit.md` → `history/` as dated artefacts; `CLAUDE.md` and `CONTRIBUTING.md` links follow. `kit/` becomes live controls only. | done — cost no CLAUDE.md edit |
...
| **8** | law — **human decision** | One sentence: *Core is Rust `no_std` only. Steward tooling — hooks, coord scripts, validators, Layer III viewers — may be any language, is never a Core dependency, and never lives under `src/`.* Closes downstream F11 and the question `tools/hologram-ir-shape-check.py` raises. | go phase 8 |
...
### 4.7 Phase 4 — expanded, after measurement

Measured at `5c9e394`. **The table's instruction was wrong and phase 4 is
changing because of it.** It said: *atomic primitives become `AP0–AP4`;
manifest pieces are written `Piece 1`, never `P1`; audit stations `S-A1`.*
That is a rename migration. The measurement says do not attempt it.

**Why not.** Three things, each sufficient on its own.

*The scale.* These are not rare tokens. `A1` appears in **50 files**, `C1` in
42 (143 uses), `D1` in 40 (169 uses), `S0` in 33 (160 uses), `P4` in 31. A
rename touches more files than every restructure phase so far combined, across
stations nobody holds, while two campaigns are writing into those same shelves
hourly.

*The keys are baked.* `contracts-axle.js` and its siblings carry `"A1"`…`"A9"`,
`"B1"`…`"B5"`, `"C1"`…`"C10"`, `"X1"`…`"X6"`, `"t0"`…`"t6"` as **string
literals**, and [the 1:1 audit](../clock/gearing-code-1to1.md) states that the
47 names *"form a bijection with the registry"*. Renaming a gear tooth breaks
running code and a documented audit at once. The two golden IR fixtures
likewise carry `P1`–`P15` as id literals.

*No demonstrated harm.* R2 is a **hazard** finding, not an injury one. The one
real mis-citation this session — `B10` written where `B11` belonged, caught by
an outside reviewer — was *within* a single family, which a rename would not
have prevented. Migrating 50+ files to fix a collision that has not yet caused
a recorded error is the cure being worse than the disease.

**The collision, precisely.** 33 tokens are claimed by two or more families;
the worst are claimed by three:

| Token range | Families claiming it |
|---|---|
| `A1`–`A9` | Graphic A panel · gear `trainA` tooth (baked) · coherence-audit station |
| `C1`–`C5` | Graphic C panel · gear `trainC` tooth (baked) · Coffee Cup stage |
| `C6`–`C10` | Graphic C panel · gear `trainC` tooth (baked) |
| `B1`–`B5` | Graphic B panel · gear `trainB` tooth (baked) |
| `P1`–`P4` | atomic primitive (`P0`–`P4`) · manifest Piece / IR `pieces[]` |
| `X1`–`X6` | gear audit key (baked) · crate-map Page X |

Out of scope: identifiers that are never cited across files. `T1`–`T5` appear
in 50 files but mean a **ticket number inside one pass plan** in most of them
and **rope tension** in the Kauffman clipboard (`T2 ≤ κ T1`). Locally scoped,
locally unambiguous, no register entry needed.

| Sub-phase | Station | Work |
|---|---|---|
| **4a** | maps | `docs/namespace-register.md`: every identifier family — range, owning station, defining document, and whether it is baked into code. Then the collision table above, with **how to tell** for each. Then the citation rule. |
| **4b** | law | One row in `docs/README.md` and one line in `AGENTS.md`, so the register is reachable from the court door and from the file every agent loads first. |

**The citation rule, which is what phase 4 actually delivers.** A bare token
stays legal where its context fixes it — inside `contracts-trainA.js`, `A5` is
a tooth and nothing else; inside the close reading, `D1` is a panel. **When a
token could be read as another family's, qualify it**: `Graphic A5`,
`trainA:A5`, `audit station A5`, `Piece 5` versus `P4`. Each station applies
the spelling when it next touches a file it owns. **No file is rewritten for
this.**

**No check.** A check flagging bare ambiguous tokens would fire thousands of
times on correct prose and be switched off within a day — the same failure mode
check 7 nearly had. The register is documentation, and its enforcement is a
reviewer noticing an unqualified cite, not a hook.

**Order.** `4a` before `4b` — the register must exist before anything links it.

**Accept phase 4** when: the register lists every family with its owner and its
baked-or-not status; the 33 collisions are named with a way to tell them apart;
the citation rule is stated; the register is linked from the court door and
AGENTS.md; **and no existing identifier has been renamed.**

### 4.8 Phase 4 — outcome, measured

Ran 2026-09-18. Two commits: `d637523` 4a (maps), `e2ca29f` 4b (law).

| Acceptance criterion | Result |
|---|---|
| Every family listed with owner and baked status | **16 families** |
| Collisions named with a way to tell them apart | **33 tokens, 9 ranges** |
| Citation rule stated | yes |
| Register linked from the court door and AGENTS.md | linked from **4 documents** |
| **No identifier renamed** | **none** — verified by diffing my own two commits |

The whole of phase 4 touched four files: the register, one row in the court
door, one clause in AGENTS.md, two station files. `atomic-primitives-map.md`,
`systems-manifest.md`, `docs/gearing/`, `docs/clock/` and the IR fixtures were
not opened. That is the acceptance criterion, not a side effect.

**Measuring found two collisions the plan had not catalogued.** `R1` is both
the gear escapement key — baked in `contracts-escape.js` — and restructure
finding 1. And the manifest's open questions **carry no prefix at all** in the
source: they are bare numbers 1–12. The `Q9` that four documents cite them by
is a downstream invention, now recorded as such.

**The rule is a reader's rule, not a writer's burden.** A bare token stays
legal where its context fixes it; qualify only across shelves, where `A5` could
be a panel, a tooth or a station. The tells are mechanical and worth keeping in
mind: `trainA` stops at `A9` while audit stations run to `A14`; Cup stages stop
at `C5` while Graphic C runs to `C12`; the zero-padding in `C01` versus `C1`
and `F01` versus `F1` is load-bearing and should not be normalised away.

**No hook check, deliberately.** A check flagging bare ambiguous tokens would
fire thousands of times on correct prose and be switched off within a day —
the failure mode check 7 nearly had, and the reason check 8 was scoped to doors
rather than to citations.

The register closes by saying what it is not: it grants no authority, and a
citation is still checked against its source rather than against that page.

**Phases 0–4 are complete.** Filing, doors, law copies, checks and naming are
done. What remains is phase 5, and phase 5 is now the entire problem: at close,
`docs/clock/` stood at **0/43 door coverage**, having grown from 34 files when
it was first deferred. It is growing faster than any phase has cleared it.

### 4.9 Phase 5 — expanded, after measurement

Measured at `f282dac`, 2026-09-19 00:0x UTC. Phase 5 was deferred four times
and the shelf grew each time: 34 files when first deferred, 39, 41, 43, and
**48 now**. Measuring it found that the delay changed what it is.

**`docs/clock/` is no longer one shelf.** It holds four different kinds:

| Kind | Count | Examples |
|---|---|---|
| Shadow prose products | 12 | `shadow-clock-*`, `philosophy-map.md`, `system-mathematical-model.md` |
| Gear manifests | 4 | HCC-A, Coffee Cup, Water, Train H×C×W |
| Executed pass plans | **28** | six campaigns: coffee-cup, hcc-a, water, train, projection-visual, projection-merge |
| **A web application** | 5+ | `lace-projection.html`, `sw.js`, `projection-bake-worker.js`, `manifest.webmanifest`, `wasm/` |

A service worker and a web manifest are not Shadow prose. That shelf acquired
an app while the restructure was working on doors.

**R4 was too strong, and the measurement says so.** I recorded "one file per
pass" as the defect. It is not. **18 of 39 executed pass plans are cited by
another document** — the clipboards campaign links its pass plans *from its
product*: `kauffman-4ed-clipboard.md` cites six of its own. The clock gear
manifests cite **zero**. The defect is not the pattern; it is **a product that
does not link its own provenance**. Clipboards already models the fix.

**A renderer regression, same class as the court-root one.** Phase 0 named
three viewers explicitly, so the fourth fell through: `lace-projection.html`
resolves to `hologram`, while the other three resolve to `renderer`. Its
runtime assets — service worker, bake worker, web manifest, `wasm/` — are
`hologram` too. A station that owns three of four viewers does not mean
anything.

**The campaigns appear to have stopped.** `train-pass-6-plan.md` is headed
*"Stop stamp"*; last clock commit 23:07, last ASCII commit 23:25, Kauffman
16:24. Phase 5 should still be written so a resumption lands correctly rather
than assuming quiet.

| Sub-phase | Station | Work |
|---|---|---|
| **5-pre** | coord — **policy call** | Make the viewer rule generic so a fifth viewer cannot fall through. Either **all `*.html` → renderer** (and the PWA runtime assets with it), or renderer is retired and viewers belong to their shelf. Recommended: the first. Flagged because the hologram agent is actively building that app, and this changes who may edit it. |
| **5a-pre** | maps | This plan's two links into `water-pass-5/6-plan.md` become bare filenames, so 5a does not strand them. |
| **5a** | hologram | `git mv` the 28 pass plans to `docs/clock/passes/`, with `passes/README.md` indexing them by campaign. The clock door then lists products and points at that index once, instead of 28 rows it will never keep current. |
| **5a-post** | maps | Re-link the two citations at their new path. |
| **5b** | hologram | The four gear manifests gain a provenance line linking their own pass plans — the clipboards model. Retitle them `Gear manifest — X (Shadow)` so four documents stop sharing the manifest's name (R3). |
| **5c** | clipboards | The clipboards door links its 20 files (the deferred 1h). Its pass plans stay flat: 11 files with products that already cite them do not need a subdirectory. |
| **5d** | maps | `systems-manifest-ascii.md` links its own six pass plans. Same rule, same reason; no subdirectory at six files. |

**Why a subdirectory for clock and not for the others.** Linking is what makes
a file reachable; a subdirectory is what makes a shelf legible. At 28 of 48
files the clock shelf has lost legibility, and the move earns its cost. At 11
of 20 and 6 of 19, clipboards and the court root have not. Applying a uniform
rule here would be tidiness, not a fix.

**The forward rule, which is the durable half.** Each door states: *a pass plan
lives beside the product it serves, and the product links it when the pass is
...
main red between them.

**Accept phase 5** when: no `*.html` falls outside the viewer rule; the clock
shelf reads as products plus one `passes/` index; each of the four gear
manifests links its own passes and no longer shares the manifest's title; the
clipboards and court-root products link theirs; and every door states where a
new pass plan goes.

### 4.10 Phase 5 — outcome, measured

Ran 2026-09-19. Six commits across three stations, while the projection
campaign was live — a seventh pass landed between the plan and the move.

| Measure | Session start | Phase 5 start | Now |
|---|---|---|---|
| Orphans, path-resolved | 62 | 34 | **1** |
| Advisory findings | 45 | 40 | **5** |
| `docs/clock/` door coverage | 0/43 | 0/48 | **complete** |
| `docs/clipboards/` door coverage | 1/17 | 1/20 | **complete** |
| Documents titled "Systems Manifest" | 4 | 4 | **1** |

The single remaining orphan is `.gitignore`. Door coverage is complete
everywhere except `prompts/` 0/16 (station held), `hologram/` 16/23 (the seven
primitive sheets reach through the catalog), and `coord/` 0/1.

**What the shelf turned out to need.** Thirty pass plans moved to
`docs/clock/passes/`, indexed by campaign, each campaign naming the product it
served. The clock door had *named* eighteen files and linked none of them,
reading as a complete index while covering nothing — the PWA that arrived
during the restructure was listed as nothing at all. Four documents titled
"Systems Manifest" competed with the Hands manifest; they are gear manifests
now, one gear each.

**R4's actual fix, in place.** Each gear manifest links its own executed
passes. Coffee Cup, HCC-A, Water and Train cited **zero** of their own
provenance before this; the clipboards campaign had been doing it correctly all
along, which is how the measurement found the real defect.

**The forward rule is in every door**, which is what makes this survive a live
campaign: *a new pass plan goes here, and the product it serves links it once
the pass is executed.* `docs/clock/` adds "here means `passes/`".

**Three mistakes, recorded because the plan is also a record of how it went.**

1. `1b27939` (5a-pre) claims to have unlinked three citations and changed
   nothing but its own station file. The `CITED` measurement used
   `git grep -l`, which matches any occurrence, so backticked **mentions** were
   read as links. Two commits were planned around a problem that did not exist,
   and `5a-post` was never needed.
2. 5b matched pass plans by each gear manifest's filename stem. The train gear
   is `train-h-c-w-systems-manifest.md` and its passes are `train-pass-N`, so it
   reported *"0 passes linked"* — and I took the number as an answer rather than
   a question. Three gears got provenance and the fourth silently did not, until
   a follow-up check caught it.
3. A Python f-string backslash error killed 5b after its door rewrite had
   already been written to disk, which is why 5b is two commits.

**5-pre is not done, and the recommendation has changed.** §4.9 proposed *all
`*.html` → renderer*. Reading [`gearing/CLAIMS.md`](../gearing/CLAIMS.md)
again: the renderer station existed for **one** artifact — the *frozen*
`shadow-clock-gearing.html` under designated-editor discipline. **Phase 0
expanded it to three viewers, and that expansion is what made it incoherent.**
Forcing the hologram agent to claim `renderer` to edit an app it is actively
building would be friction this restructure introduced. The alternative —
narrow `renderer` back to the frozen artifact and let each shelf own its own
viewers — is now at least as defensible. Left for the human rather than
silently reversed.

### 4.11 Phase 6 — expanded, after measurement

Measured at `b5f46f1`. **Phase 6 should not run as written, and most of what it
was for is already done.**

It said: move `READ_ME_FIRST.md` and `downstream-audit.md` into `history/` as
dated artefacts, so `kit/` becomes live controls only. Both halves fail.

**`downstream-audit.md` is not historical — it is live.** Ten documents link
it, including `CLAUDE.md`, `.claude/README.md`, [the namespace
register](../namespace-register.md), `agent-control.md` and [the coherence
audit prompt](../prompts/coherence-audit-prompt.md) — whose **station A14
exists to re-check its F01–F12 at the live tip**, and did, producing a status
table in [the log](../coherence-audit-log.md). `history/` is for *"reading
notes against current law … not a source of record"*. Filing a live register
there, and breaking ten links including one in `CLAUDE.md`, would misplace the
one document the audit is built around.

**`READ_ME_FIRST.md` is dated but load-bearing.** `CONTRIBUTING.md` routes
contributors through it as the package door. A package door is not a recovery;
`history/` is the wrong shelf for it, and the move breaks the route.

**R10 is already addressed.** The finding was *"`kit/` mixes live controls with
dated artefacts"*. Phase **1g** gave the kit door a status column that says
exactly which is which — `READ_ME_FIRST` **STALE** at pin `4fe984b`,
`downstream-audit` a *dated snapshot, re-checked by the coherence audit*,
`agent-task-template` *live*, the other two transport copies. Labelling solved
it. Relocation would re-solve it by breaking things.

**The design those two files use is correct and worth naming**, because the
next dated artefact should copy it: *the audit is frozen at its pin; its live
status lives in the append-only log.* F10 records the emission draft as missing at that pin
because that was true at `4fe984bb`, and A14 records the live reading — *file
now exists as DRAFT, acceptance still missing*. Two documents, one dated and
one live, neither pretending to be the other.

**One real remainder.** That same F10 row is the last stale-absence advisory in
the tree, and it is a **false positive of my own check**: the claim is dated by
the document's header, but the check only looks for a revision within one line
of the claim. Two ways to close it:

| | Fix | Cost |
|---|---|---|
| **a** | The F10 row names the pin it was true at — one line, in the row, where a reader checking the claim is already looking | a dated row gets more honest; no check change |
| **b** | The check learns document-level pins: a header declaring a revision dates every absence claim in the file | more general; risks silencing genuinely stale claims in any long-lived document that happens to carry a pin |

Recommended: **(a)**. It makes the row say what it means, and it does not
weaken a check across every document to fix one line.

| Sub-phase | Station | Work |
|---|---|---|
| **6a** | kit | The F10 row cites the pin it was true at. Clears the tree's last stale-absence advisory at its source. |
| — | — | **Nothing moves.** `READ_ME_FIRST.md` and `downstream-audit.md` stay in `kit/`. R10 closed as addressed by 1g. |

**Accept phase 6** when: the F10 row is dated, the stale-absence advisory is
gone, `CLAUDE.md` and `CONTRIBUTING.md` are untouched, and the plan records
that the move was rejected on measurement rather than left ambiguous.

### 4.12 Phase 6 — outcome, measured

Ran 2026-09-19. **One commit, `20a8b61`, one line changed, nothing moved.**

The phase as written would have relocated two files, edited `CLAUDE.md` and
`CONTRIBUTING.md`, and broken ten links. What it actually needed was a date on
one claim. R10 had been closed by 1g's status column since phase 1; the rest of
phase 6 was solving a problem that no longer existed.

F10 now reads *"At the audit pin `4fe984bb`, …"*, which clears the tree's last
genuine stale-absence advisory at its source rather than by weakening the check
across every document.

**A gap in my own check, now on its third instance.** The check cannot tell a
claim from a *quotation* of a claim. §4.11's description of F10 tripped it, as
§4.5's description of `agent-control.md` did before, and the phase-3 plan's
before that. Each time I have reworded my prose — including here — and the
gap has stayed. **The reword is a workaround, not the fix.** The fix is for the
check to skip spans inside backticks or a blockquote, and it is outstanding
work for the kit station, recorded here so the next reword is recognised as the
fourth dodge rather than a new idea.

**The doors are already drifting, and check 8 is what says so.** In the eight
minutes between phase 5 closing and phase 6 running:

| | Phase 5 close | Phase 6 run |
|---|---|---|
| `docs/clock/passes/` | 30/30 | **30/33** |
| `docs/clipboards/` | complete | **22/23** |
| Orphans | 1 | **4** |

Three pass plans and a clipboard landed without their index or product linking
them. **The forward rule is stated in three doors and is being skipped**, which
is the honest result: stating a rule is not enforcing it, and a shelf under an
active campaign drifts by a few files an hour.

That sharpens a question phase 7 must answer, and it is the human's: **check 8
is advisory — should linking your own pass be a gate?** Failing the gate for
the shelf you just committed to would stop the drift where it starts, and would
also sometimes block an agent mid-campaign. That is a protocol change, not
filing.

**Phases 0–6 are complete but for `5-pre`.** Every remaining phase — `5-pre`,
7, 8 — changes law, protocol, or another agent's access, and none should be
taken by an agent alone.
