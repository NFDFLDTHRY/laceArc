...
| **6** | kit | **Re-scoped in §4.11 — nothing moves.** The move was wrong: downstream-audit is live, not historical, and R10 was already closed by 1g. One dated row remains. Original text: | `READ_ME_FIRST.md` and `downstream-audit.md` → `history/` as dated artefacts; `CLAUDE.md` and `CONTRIBUTING.md` links follow. `kit/` becomes live controls only. | go phase 6 — costs a CLAUDE.md edit |
...
status lives in the append-only log.* F10 reads "Pointer-emission.md is absent"
