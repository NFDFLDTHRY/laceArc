...
| **4** | maps + law | `docs/namespace-register.md`, and links to it. **Superseded in §4.7: no renaming.** The keys are baked into `contracts-*.js` and the IR fixtures, `A1` alone spans 50 files, and the collision has caused no recorded error. Register + citation rule instead. | see §4.7 |
