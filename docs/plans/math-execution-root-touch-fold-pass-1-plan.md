...
[
mathsf{Arrive}(v):Lmapsto L^smallfrown(mathsf{WORD},v)
]
...
[
ho_{L_s}(v)=	ext{the seeded WORD-root index for }v.
]

Questions Pass 1 must answer:
- Is (ho) safely a **reading of (L_s)** rather than a second root table?
...
[
v_{n+1}	ext{ seen before}Rightarrow 	ext{create touch to }ho(v_{n+1}).
]

Candidate notation only:

[
	au_{n+1}=operatorname{Touch}(ho(v_{n+1})).
]
...
igcup_{rin refs(i)}operatorname{Reach}_L(r)
]
...
[
C_{n+1}=C_ncup operatorname{Reach}(	au_{n+1})cup{p_{n+1}}.
]
...
### T7 — root lookup
(ho(v)) must be a view/read over the seed wordspace; any acceleration index is projection/cache, never authoritative state.
...
| Pass | Job |
|---|---|
| **2** | Formal seed/runtime split: (L_s), root read (ho), post-seed touch (	au); update the partial algebra only after Pass 1 dispositions |
