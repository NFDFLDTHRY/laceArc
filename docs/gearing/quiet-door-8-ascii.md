...
This diagram is a reading of the gearing door. Shafts are Layer III. This shelf is not Hands. Frozen HTML stays renderer.
...
```
  ...
  [X]  invent Φ here
  [X]  edit another shaft
  [X]  rewrite frozen HTML
  [X]  claim renderer
  [X]  claim a shaft this pass
  [X]  src/
  [X]  door #7 reopened
  [GAP] pointer-emission answers (G2)
```

Shafts are Layer III. Named scripts are not owned here. Renderer stays unclaimed.
...
|---|---|
| Layer III shafts | Core |
| named scripts | owned JS this station |
| claim / release protocol | a shaft drop |
| RESYNC FIRED/CLEAR | rewrite RESYNC |
| frozen HTML = renderer | rewrite HTML this pass |
| WebGPU 3D only | invent Φ |
| no `src/` | open Rust |
| door #7 CLOSED | reopen Hologram |
