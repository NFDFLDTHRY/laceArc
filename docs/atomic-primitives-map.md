...
**Current-law qualification — SAMPLING / DENSITY / EMERGENCE Pass 6.** This map predates the later sampling reconciliation. Read dated G2/source-silence statements as historical unless qualified here. Current facts are: every successful arrival appends WORD; unseen WORD becomes root; seen occurrence RootTouches root; POINTER is binary Join over two ordered earlier same-line WORD/POINTER points; repeated Join can ground finite samples; H7–H12 establish derived 2D sampling as the relationship-discovery mechanism and star/search as derived read-only structure. **The remaining Core-adjacent gap is exact sampling-derived retention/materialization.** G1/tokenization and other independent obligations remain open. The reconciled pointer-emission draft is still not human-accepted, so no implementation authorization follows.
...
| 9 | Dictionary wiring | Definition text uses the governing rule and sequence-as-route through ordinary roots/stars; sampled/retained structural participation uses Join. | G1; exact retention/materialization; identifying/traversing a particular definition G3–G4. | A6, A11; B4; C9; current law |
