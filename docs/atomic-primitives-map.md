...
| P4 | **APPEND** | Growth effect. Add a new section at the end, preserving every earlier section and position. Current law fixes the binary Join constructor once retained operands are addressable; exact sampling-derived retention/materialization remains open. | More cord and another pass; previous passes remain. | B1, B12; C1, C12; D1 and footer; Piece 1–2, 6 |
...
This table covers **all 15 manifest pieces**. A composition here identifies structural dependencies. Current law fixes physical Join and the sampling discovery architecture; rows that historically named G2 are dated selector-era language. The active unresolved seam is **sampling-derived retention/materialization** unless another independent gap is named.
...
| 2 | Governing rule | Arrival contributes a new P2 occurrence to P0; unseen becomes root; seen occurrence RootTouches root through P3; sampling discovers multi-scale structure; retained relations use binary Join. | Exact sampling-derived retention/materialization remains open. No independent graph-building phase follows ingest. | B12; C12; D footer; H7–H12; current law |
...
| 5 | POINTER entry | P3 is binary Join(a,b), appended by P4 with two ordered earlier P1 positions under K3; either operand may be WORD or POINTER. | Exact retention/materialization remains open; no semantic chooser is imported; adjacency alone is closed false. | D1–D3, D6; H7–H12; current law |
...
| 7 | Star formation | First occurrence is the root; later occurrences remain distinct and RootTouch that root; later Join ancestry participates in the formation. Depends on P1–P4, not a Star record. | General identity G1; exact retention/materialization; faithful structural readout G5. Star search is derived, not stored. | A2, A5, A12; B2–B3; D4–D5; H7–H12; current law |
| 8 | Sequence as route | Ordered occurrences on the continuous strand give the word route through formations. No sentence container is added. | Adjacency alone does not emit Join. Sampling discovery is separate from arrival order; exact retention/materialization remains open. | A4; B5; C2; H7–H12; current law |
| 9 | Dictionary wiring | Definition text uses the governing rule and sequence-as-route through ordinary roots/stars; sampled/retained structural participation uses Join. | G1; exact retention/materialization; identifying/traversing a particular definition G3–G4. | **B4; C9; A11–A12; current law** |
...
| 11 | Touch / re-entry | Seen-word RootTouch is Join(new occurrence, root); any P3 Join remains addressable and may participate in later Join. Existing sections remain. | Exact sampling-derived retention/materialization; read-versus-new-arrival distinction G4. | A10; B7–B8; C7; D6; H7–H12; current law |
| 12 | Dimension | Read independent participation and relations among relations in the same P0. Join may name prior Join; N-D remains a projection. | Axis/family identification G5 remains open. The later 2D sampling working surface is distinct derived machinery; retention/materialization remains a separate open seam. | C5–C8; D6; H7–H12; current law |
| 13 | Projection | Read P0's order, occurrences, and references into a sequence, graph drawing, or 3D view under K5. | General traversal G4; faithful geometry/readout G5. | **B11; C2–C4, C11; D4** |
| 14 | No stored meaning / no collapse | K1–K5 constrain every preceding composition. Ambiguity remains as distinct passages. | No new mechanism is needed to enforce this at specification level; implementation enforcement is unproved. | **A8, A12–A13; B10; C9, C12; D** |
| 15 | Star-internal traffic | Observe root, occurrences, RootTouch and later Join ancestry with order and occurrence distinctions preserved. | Precise readout G4–G5; derived star-search frontiers are mapped; exact retention/materialization remains open. | A12; B9; H7–H12; current law |
...
| Clock tick | One legal append, P4, as counted in the live law. | Neither elapsed time nor a holder's request supplies the retention/materialization rule. The Join constructor and sampling discovery architecture are separate. |
...
**[UNESTABLISHED] Full automatic retention/materialization and minimality.** Current law supports WORD/root/RootTouch/binary Join plus sampling discovery. It still does not provide the exact universal rule retaining every sampled relation, a faithful reconstruction algorithm for all geometric distinctions, or a proof that this basis is algebraically minimal. Removing occurrence distinction loses the two PIE passages; removing POINTER addressability loses D1's 0007/0009/0010 participation. Those are necessity witnesses at the stated grain, not a general completeness theorem.
...
| G2-current — sampling-derived retention/materialization | What exact condition retains sampled structure, and how is an ephemeral sample grounded when a retained operand needs an address? RootTouch is closed; binary arity, target kinds, backwardness, repeated-WORD admission, and finite-sample construction are settled. | Current manifest / pointer-emission reconciliation; H7–H12. |
...
Closing the old broad G2 constructor bundle no longer closes every implementation obligation. The remaining retention/materialization gap is independent of G1 and G3–G7. The mathematical model §6's executable-incompleteness “if and only if” wording is too strong to use as a sufficiency result: control C03 explicitly keeps other facts needed by an implementation open. No such sufficiency claim is made here.
...
| Known source limitations | Current law fixes binary Join; historical unary POINT is not imported. Membership is not full star structure. Unbounded extension does not by itself prove infinite traversal. Constructor + sampling closure is not sufficient for retention/materialization or every implementation obligation. |
