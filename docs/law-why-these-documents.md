...
HCC-A, The Coffee Cup, and Where the Water Is Loud / Dual Proofing Systems are **conceptual reference / behavioral-goal sources**. They establish both (a) target families of cognition, causality, communication, and behavior and (b) **source-defined desired-behavior mechanisms**: functional/causal relations, state distinctions, feedback, invariants, transitions, parameterizations, and interface behavior that are part of what the target system is meant to do. The repository is a **lab for discovering, testing, and rejecting concrete systems capable of realizing those obligations against the Lace substrate**. **The goal documents do not thereby specify the exact software objects, storage representation, holder architecture, scheduling, or Lace/Core implementation that must realize them.** A source behavioral mechanism is therefore stronger than a decorative analogy but weaker than a concrete implementation prescription. The repo's additional H/C/W gears, shafts, clutches, train meshes, and other cross-source correspondences remain interpretations/hypotheses unless the original defines them.
...
**Four roles, clarified.** **Materials** — the academic documents, toolchain/web specifications, and other descriptive sources used to reason and test. **Behavioral goals / desired-behavior mechanisms** — HCC-A, The Coffee Cup, and Where the Water Is Loud: the originals define both observable target behavior and functional relations the target behavior depends on. **Substrate under investigation** — Lace: `docs/graphics/`, then `docs/systems-manifest.md`. **Lab** — this repository's experiments, models, proofs, maps, harnesses, and candidate architectures used to discover **how** those functional obligations can be realized while preserving substrate law. The concrete implementation systems are experimental unknowns until evidence establishes them. The earlier “parts of it” wording does not predeclare literal runtime parts.
...
**What it does not license.** Reading *"not a constraint on the substrate"* as *"establishes nothing"* is a category error. **The reference texts establish behavioral targets and source-defined desired-behavior mechanisms; the lab explores the concrete machinery required to realize them.** An explicit source invariant such as HCC's parallel RIC/PFC relation, Coffee Cup's causal stage distinctions/recurrence, or Water's two proofing systems + interface is not demoted to mere analogy merely because its eventual code/data layout is unknown. Conversely, a repo correspondence, holder decomposition, state object, storage layout, sampling adapter, or H×C×W coupling is not architecture merely because it resembles the source. **Concrete realization mechanisms and cross-source couplings remain proposals until evidence and the relevant authority establish them.**
