...
**Current bounded register, 2026-09-20:** **49 registered family rows**, **371 distinct literal tokens**, and **125 tokens owned by two or more rows**: 80 have 2 owners, 25 have 3 owners, 8 have 4 owners, 7 have 5 owners, 5 have 6 owners. There are 578 row-token memberships. `A5` has six registered owners: Graphic A panel, `trainA` tooth, coherence-audit station, AgentScope paper section, attachment request, and model adversarial section.
...
**Historical statistics, preserved as reports:** the previous headline said 30 families, 77 colliding tokens, 14 with three owners and 5 with four. Earlier readings recorded derivations at `810d200`, `af89f85`, `dbeaa88`, and verification iterations 4–5; reported family corrections 26 → 25 → 27, collision corrections 71 → 72 → 77, and a failure to reproduce an earlier 60. Those are dated register history, not current totals. The former projection denominator of 30 described the iteration/interaction plans; the complete pass-plan shelf now contains 60 files. The present literal-token tally does not preserve mistaken aliases merely to reproduce a headline.
...
| **Emission morphism** | `Φ`, `Φ-morphism`, `Φ-schedule` | [pointer-emission.md](pointer-emission.md) §Φ | law | **Not one token — one named, two halves.** `Φ-schedule` is the *when* and is `[GAP]`; `Φ-morphism` is the shape and is **stated**. **A bare `Φ` written before 19 September 2026 means the schedule**, because that was the only half the tree had a name for |
...
| Partial model section | `S0`–`S8` | [partial algebra](math-execution-model.md) | maps | Top-level section headings; decimal subsection labels are not separate family tokens in this tally |
...
| `K1`–`K6` | Atomic constraint · Kauffman section | Name the atomic map or clipboard; Kauffman extends to K9 |
...
| `T1`–`T9` | Legacy ticket/tension row · target fact ledger | Name the pass-local ticket, Kauffman tension or `target T1`. Full `M-T3` / `P-T3` task identifiers retain their prefixes and are not literal `T3` |
...
| `S0`–`S7` | HCC phase · partial model section | Name the conceptual HCC phase or the partial-algebra section; the latter continues through S8 |
...
**Coverage limit:** this update registers the reviewed reduction, target, iteration-6/7 and mathematical-model product families above, including their verified top-level headings and named claim/specimen rows. It does not expand every unrelated campaign key or decimal subsection label. The tally is exact for this table, not a full-repository identifier census or a claim that no other family collides.
