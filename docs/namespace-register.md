...
Thirty identifier families share one small alphabet. **77 tokens are claimed by two or more families**, 14 of them by three and **5 by four**. `A5` is a Graphic A panel, a `trainA` gear tooth, a coherence-audit station, and a section of the AgentScope paper.
...
**The published figure of 26 families was the duplicate below counted twice; the true count was 25, and adding two families makes it 27.** Re-derived again at iteration 5 pass 3, **72 → 77**, the five being `Q1`–`Q5`. Running one script over the table before and after iteration 4 pass 3's edits gave **71 → 72** two-owner tokens — `i1`–`i3` gained a second owner, and `1`–`12` lost one when the duplicated row was merged onto the `Q` form. **The previously published 60 could not be reproduced by any reading of this table**, which is the failure this register already names twice elsewhere: a headline derived by a method nobody wrote down. The script is now the method.
...
| `Q1`–`Q5` | **manifest open question** · **emission open question** | **Two owners, and the confusable pair is real.** Manifest `Q1` is *tokenization* and emission `Q1` is *when is a POINTER appended*; manifest `Q2` is *the POINTER emission algorithm* and emission `Q2` is *what is the arity*. **Both families talk about pointers**, so context does not reliably fix them. Write `manifest Q2` or `emission Q2`. `Q0` is emission only; `Q6`–`Q12` are manifest only |
| `I`, `II`, `III` | **boundary stamp** · **prefix-device numeral** | **Two tells and both hold.** A boundary stamp is bracketed — `[I]`, `[II]`, `[III]`. A prefix-device numeral is noun-prefixed and bare — `Collision I`, `Sheet II`, `slot I`. Nothing in the tree is ambiguous today; the row is here so it stays that way |
